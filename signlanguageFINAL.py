...
import threading  # To read sensor data
...
# Trigger variable to enable or disable gesture detection
sensor_trigger_active = False  # Initially False
...
# Store sensor state
sensor_triggered = StringVar()
sensor_triggered.set("No Touch") # Default state when no signal is detected

# Read sensor data in a separate thread
def read_sensor_data():
    global arduino, sensor_trigger_active
    while True:
        if arduino:
            data = arduino.readline().decode("utf-8").strip()
            if data == "TOUCH DETECTED":
                sensor_trigger_active = True  # Enable gesture detection
                sensor_triggered.set("Sensor Triggered!")  # Update status
            else:
                sensor_trigger_active = False  # Disable gesture detection
                sensor_triggered.set("No Touch")  # Update status
...
# Sensor status label
sensor_status_label = Label(win,textvariable=sensor_triggered,font=("Arial", 18, "bold"),bd=5,bg="#20262E",width=30,fg="#F5EAEA",relief=GROOVE)
sensor_status_label.place(x=580, y=750)
...
def select_img():
    ...
    if gesture:
        cv2.putText(frame, gesture, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (46, 38, 32), 2, cv2.LINE_AA)
